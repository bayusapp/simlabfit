...
class M_Model extends CI_Model
{
  ...
  {
    $this->db->select('honor.id_honor, matakuliah.kode_mk, matakuliah.nama_mk, asprak.nim_asprak, asprak.nama_asprak, asprak.norek_asprak, asprak.nama_rekening, asprak.linkaja_asprak, periode.bulan, honor.nominal, honor.opsi_pengambilan, honor.status, honor.bukti_transfer, honor.no_pk');
    $this->db->from('honor');
    $this->db->join('daftar_mk', 'honor.id_daftar_mk = daftar_mk.id_daftar_mk');
    $this->db->join('matakuliah', 'daftar_mk.kode_mk = matakuliah.kode_mk');
    $this->db->join('asprak', 'honor.nim_asprak = asprak.nim_asprak');
  ...
  {
    $this->db->select('honor_aslab.id_honor_aslab, honor_aslab.nominal, honor_aslab.status_honor, honor_aslab.opsi_pengambilan, honor_aslab.bukti_transfer, aslab.nim, aslab.namaLengkap, aslab.norek, aslab.nama_rekening, aslab.linkaja, periode.bulan');
  ...
  {
    $this->db->select('DISTINCT asprak.nim_asprak, asprak.nama_asprak, matakuliah.kode_mk, matakuliah.nama_mk, substring(jadwal_lab.jam_masuk, 12, 5) masuk, substring(jadwal_lab.jam_selesai, 12, 5) selesai');
